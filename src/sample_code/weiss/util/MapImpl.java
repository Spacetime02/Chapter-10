...
/**
 * MapImpl implements the Map on top of a set. It should be extended by TreeMap and HashMap, with chained calls to the
 * constructor.
 */
abstract class MapImpl<KeyType, ValueType> implements Map<KeyType, ValueType> {
	 ...
	 * @param key
	 *            the key to search for.
	 * @return the value that matches the key or null if the key is not found. Since null values are allowed, checking if
	 *         the return value is null may not be a safe way to ascertain if the key is present in the map.
	 */
	...
	/**
	 * Abstract class to model a view (either key or value view). Implements size and clear methods, but not iterator
	 * method. View delegates to underlying map.
	 */
	...
	/**
	 * Class to model the key set view. remove is overridden (otherwise a sequential search is used). iterator gives a
	 * KeySetIterator (see Figure 19.81). getMatch, the nonstandard part of sample_code.weiss.util.Set is not needed.
	 */
	...
	/**
	 * Class to model the value collection view. Default remove which is a sequential search is used. iterator gives a
	 * ValueCollectionIterator (see Figure 19.81).
	 */
	...
	/**
	 * Returns the keys in the map. These semantics are different from those in java.util because in this class, changes
	 * made to the returned key set do not cause changes to be reflected in the map.
	 * 
	...
	/**
	 * Returns the values in the map. There may be duplicates. These semantics are different from those in java.util because
	 * in this class, changes made to the returned value collection do not cause changes to be reflected in the map.
	 * 
