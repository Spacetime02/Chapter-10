...
public class Arrays {
	...
	/**
	 * Performs a search on sorted array arr. If arr is not sorted, results are undefined. Implicitly, this assumes that all
	 ...
	 * @return if x is found, returns the index where it is found; otherwise, x is not found. In that case, a negative
	 *         number is always returned, and this number is equal to -( p + 1 ), where p is the first position greater than
	 *         x. This can range from -1 down to -(arr.length+1).
	 ...
	 *            the comparator.
	 * @return if x is found, returns the index where it is found. otherwise, x is not found. In that case, a negative
	 *         number is always returned, and this number is equal to -( p + 1 ), which p is the first position greater than
	 *         x. This can range from -1 down to -(arr.length+1).
	 * @throws ClassCastException
	 *             if comparsions cannot be performed.
	 */
